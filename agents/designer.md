...
## Step 5 — Coverage report (machine-readable)

Write `designs/coverage.md`. This is the bridge between the designer and `/tasks`: it lists every prototype's top-level regions in a parseable format so re-running `/tasks` can detect regions you introduced beyond what `plan.md` enumerated, and emit tasks for them.

Without this file, regions you added (e.g. a Portefeuille summary card the plan didn't list) silently disappear from the implementation pipeline because no task ever gets written for them.
...
Generated by the designing-interfaces agent. One section per prototype, listing top-level regions. The `/tasks` skill reads this to ensure every region is covered by an implementation task. Re-run `/tasks` after any change to a prototype's region set.
...
- One bullet per top-level region — anything that's a `<section>`, `<aside>`, `role="region"`, or a top-level direct child of the page's main grid/flex container. Sub-components inside a region (a button inside a card, a row inside a table) do NOT get their own bullet.
- The `<ComponentName>` is the PascalCase name the developer should use when implementing it. The `/tasks` merge step matches by this exact string, so be consistent across cycles — once you've named a region `PortefeuilleSummaryCard`, do not rename it to `PortfolioCard` in a later cycle.
- The one-line purpose is for humans reviewing the diff; `/tasks` ignores it.
- Every prototype file you wrote in Step 4 must have a `## designs/<file>.html` section. Missing prototypes here = orphaned regions.

The design orchestrator handles logging and cycle management — do not write to any tracking files beyond `designs/`.
