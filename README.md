...
**Scoped diff (v0.20.0+).** Running pixel-diff + dom-diff against 20 routes when this cycle only touched one is wasted work. `scripts/routes-to-diff.mjs` reads `git diff --name-only` (against the merge-base with `main`) plus the prior cycle's failed routes, and emits the list of routes the evaluator actually needs to test. Both diff scripts accept `--only-route /foo` (repeatable) to honour that list. Policy: any change under `src/app/<segment>/page.*` (or `pages/<segment>.*`, or that segment's `layout|loading|error|route` files) maps to that specific route; any change to a shared component (`src/components/`), lib (`src/lib/`), hook, global CSS, theme tokens, or build config falls back conservatively to `*` (test all). Prior-cycle failed routes are always included regardless of file changes — a stuck route MUST be re-verified each cycle until it passes. The output JSON gets `"scoped": true` and `"scoped_routes": [...]` so the evaluator can see at a glance whether it ran a partial sweep.
